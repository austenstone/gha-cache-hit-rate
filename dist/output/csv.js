...
export class CsvOutputFormatter {
    ...
    async outputDetailed(stats, outputPath) {
        const csvWriter = createObjectCsvWriter({
            path: outputPath,
            header: [
                { id: 'repository', title: 'Repository' },
        ...
        const allOperations = stats.workflowStats.flatMap(workflow => workflow.recentOperations.map(op => ({
            repository: `${stats.owner}/${stats.repo}`,
            workflowName: workflow.workflowName,
            runId: op.runId,
    ...
    async outputSummary(stats, outputPath) {
        const csvWriter = createObjectCsvWriter({
            path: outputPath,
            header: [
