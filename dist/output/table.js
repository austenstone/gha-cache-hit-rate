...
export class TableOutputFormatter {
    ...
    printOverallStats(stats) {
        ...
        table.push(['Total Cache Operations', stats.totalCacheOps.toLocaleString()], ['Cache Hits', `${stats.totalCacheHits.toLocaleString()} (${stats.overallHitRate.toFixed(1)}%)`], ['Cache Misses', `${stats.totalCacheMisses.toLocaleString()} (${(100 - stats.overallHitRate - stats.overallPartialHitRate).toFixed(1)}%)`], ['Partial Hits', `${stats.totalPartialHits.toLocaleString()} (${stats.overallPartialHitRate.toFixed(1)}%)`], ['Hit Rate', this.formatHitRate(stats.overallHitRate)], ['Effective Hit Rate', this.formatHitRate(stats.overallHitRate + stats.overallPartialHitRate)]);
        console.log(table.toString());
        console.log();
    }
    printWorkflowTable(workflowStats, verbose) {
    ...
    printDetailedWorkflowStats(workflowStats) {
        ...
        for (const workflow of workflowStats) {
            ...
            detailTable.push(['', 'Count', 'Percentage', 'Details'], ['Cache Hits', workflow.cacheHits.toString(), `${workflow.hitRate.toFixed(1)}%`, '✅ Perfect matches'], ['Cache Misses', workflow.cacheMisses.toString(), `${((workflow.cacheMisses / workflow.totalCacheOps) * 100).toFixed(1)}%`, '❌ No cache found'], ['Partial Hits', workflow.partialHits.toString(), `${workflow.partialHitRate.toFixed(1)}%`, '🔄 Restore key match']);
