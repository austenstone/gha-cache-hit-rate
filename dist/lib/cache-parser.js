...
export class CacheLogParser {
    ...
    parseLogContent(logText, defaultJobName) {
        const operations = [];
    ...
    parseCacheLine(line, jobName, stepName) {
    ...
    parseFollowingLines(lines, startIndex) {
        const additionalInfo = {};
        for (let i = startIndex; i < Math.min(startIndex + 5, lines.length); i++) {
            const line = lines[i].trim();
            let match = line.match(CacheLogParser.CACHE_PATTERNS.restoreSuccess);
            if (match) {
                additionalInfo.sizeBytes = parseInt(match[1]) * 1024 * 1024;
